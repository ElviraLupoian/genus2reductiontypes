\documentclass[tikz,border=2pt]{standalone}
\usepackage{tikz}
\usetikzlibrary{positioning}
\begin{document}
% Namikawa--Ueno type: III^*-I^*_l-t
% Target MRNC type: III^*-I^*_l-t
% Adapted from Tim Dokchitser picture: I^*_{{\it n}}\e({\it m})III^*
% Parameter substitution: n -> l, m -> t
\begin{tikzpicture}[xscale=0.57,yscale=0.62]
\draw[thick,shorten >=2] (-1/3,0)--(82/15,0) node[inner sep=2pt,blue,above left,scale=0.8] {2};
\draw[shorten <=-3pt] (0,0)--node[inner sep=2pt,scale=0.8,blue,right] {1} (0,3/5);
\draw (6/5,0) 
    ++(0.07,-0.13)--++(-0.26,0.5)++(0.12,-0.19)  
    ++(0,0.02)
      node[blue,scale=0.7,inner sep=1,anchor=west]{1}
    ++(0,-0.02)
    ++(-0.16,0.02)--++(0.43,0.31)++(-0.13,0.07)  
    node{$\cdot$} ++(-0.06,0.12) node{$\cdot$}
    ++(-0.06,0.12) node{$\cdot$}++(0.17,-0.1)    node[auto,inner sep=5,scale=0.7,anchor=west]{$t\!+\!1$}
    ++(-0.26,0.19)--++(0.43,0.31)++(-0.18,0.04)   
    ++(0,-0.04)
      node[blue,scale=0.7,inner sep=1,anchor=east]{1}
    ++(0,0.04)
    ++(0.18,-0.04)++(-0.03,-0.15)--++(-0.26,0.5);
\draw[shorten <=-3pt, shorten >=-3pt] (6/5,3/2)--node[inner sep=2pt,scale=0.8,blue,above] {2} (2/5,3/2);
\draw[shorten <=-3pt, shorten >=-3pt] (2/5,3/2)--node[inner sep=2pt,scale=0.8,blue,left] {3} (2/5,21/10);
\draw[thick,shorten >=2] (1/15,21/10)--(97/30,21/10) node[inner sep=2pt,blue,above left,scale=0.8] {4};
\draw[shorten <=-3pt, shorten >=-3pt] (6/5,21/10)--node[inner sep=2pt,scale=0.8,blue,right] {3} (6/5,27/10);
\draw[shorten <=-3pt, shorten >=-3pt] (6/5,27/10)--node[inner sep=2pt,scale=0.8,blue,above] {2} (3/5,27/10);
\draw[shorten <=-3pt] (3/5,27/10)--node[inner sep=2pt,scale=0.8,blue,left] {1} (3/5,33/10);
\draw[shorten <=-3pt] (2,21/10)--node[inner sep=2pt,scale=0.8,blue,right] {2} (2,27/10);
\draw (127/30,0) 
    ++(0.06,-0.17)--++(-0.22,0.64)++(0.11,-0.24)  
    ++(0,0.02)
      node[blue,scale=0.7,inner sep=1,anchor=west]{2}
    ++(0,-0.02)
    ++(-0.16,0.02)--++(0.46,0.44)++(-0.12,0.08)  
    node{$\cdot$} ++(-0.06,0.16) node{$\cdot$}
    ++(-0.06,0.16) node{$\cdot$}++(0.17,-0.12)    node[auto,inner sep=5,scale=0.7,anchor=west]{$l\!-\!1$}
    ++(-0.25,0.23)--++(0.46,0.44)++(-0.19,0.04)   
    ++(0,-0.05)
      node[blue,scale=0.7,inner sep=1,anchor=east]{2}
    ++(0,0.05)
    ++(0.19,-0.04)++(-0.04,-0.2)--++(-0.22,0.64);
\draw[thick,shorten >=2] (39/10,2)--(106/15,2) node[inner sep=2pt,blue,above left,scale=0.8] {2};
\draw[shorten <=-3pt] (151/30,2)--node[inner sep=2pt,scale=0.8,blue,right] {1} (151/30,13/5);
\draw[shorten <=-3pt] (35/6,2)--node[inner sep=2pt,scale=0.8,blue,right] {1} (35/6,13/5);
\end{tikzpicture}
\end{document}
